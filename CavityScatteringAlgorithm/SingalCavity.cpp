...
void SingalCavity::weak(Triangle &T, Vector3d v, int topsign, vector<vector<double>> &nbound, vector <complex<double>> &out, vector <complex<double>> &out5m)
{
	int nboundSize = nbound[0].size();
	complex<double> zero(0, 0);
...
Vector3cd SingalCavity::weak1(Triangle &T, Vector3d v)
{
	// 求拉普拉斯u的积分
...
Vector3cd SingalCavity::weak3(Triangle &T, Vector3d v)
{
	Vector3cd out;
