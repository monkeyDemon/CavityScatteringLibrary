...
void InhomogeneousSingalCavity::weak(Triangle &T, Vector3d v, int topsign, vector<vector<double>> &nbound, vector <complex<double>> &out, vector <complex<double>> &out5m)
{
	int nboundSize = nbound[0].size();
	complex<double> zero(0, 0);
...
Vector4cd InhomogeneousSingalCavity::weak_int(Triangle &T, Vector3d v)
{
	if (T.sgn_int == 10)
	{
...
Vector3cd InhomogeneousSingalCavity::weak1(Triangle &T, Vector3d v)
{
	throw("wait to finish");
}

Vector4cd InhomogeneousSingalCavity::weak1_int(Triangle &T, Vector3d v, bool beta)
{
...
Vector3cd InhomogeneousSingalCavity::weak3(Triangle &T, Vector3d v)
{
	throw("wait to finish");
}

Vector4cd InhomogeneousSingalCavity::weak3_int(Triangle &T, Vector3d v, bool q)
{
	// q:    true->q     false->q_
...
Vector3cd InhomogeneousSingalCavity::weak4(Triangle &T, double v2, double v3, bool beta)
{
	// beta:    true->beta     false->beta_
...
complex<double> InhomogeneousSingalCavity::fInt_f(Triangle &T, Vector3d v)
{
	complex<double> out;
	if (T.sgn == -10)
	{
...
complex<double> InhomogeneousSingalCavity::fInt_int(Triangle &T, Vector3d v)
{
	complex<double> out;
...
complex<double> InhomogeneousSingalCavity::tr(Triangle &T, Vector3d v, bool f)
{
	//f:        true->f     false->f_
...
//判断并设置三角形类型，与界面交点位置等信息
Triangle InhomogeneousSingalCavity::tri(double a1, double a2, double a3, double x1, double x2, double x3, double y1, double y2, double y3)
{
	Triangle l;
...
//按照统一规则标准化三角形坐标信息
void InhomogeneousSingalCavity::solveTri(Triangle &l)
{
	// 按照统一规则标准化三角形坐标信息
...
void InhomogeneousSingalCavity::solveTri_int(Triangle &l)
{
	if (abs(l.sgn_int) < 10)
	{
...
void InhomogeneousSingalCavity::solveTri_int1(Triangle &l)
{
...
void InhomogeneousSingalCavity::ustar_int(Triangle &T, Vector4d &u4, Vector4d &u5, Vector4d &u4_, Vector4d &u5_)
{
	double x1 = T.x1_int;
	double y1 = T.y1_int;
	double x2 = T.x2_int; 
...
void InhomogeneousSingalCavity::ustar2_int(Triangle &T, Vector4d &u5, Vector4d &u5_)
{
	double x1 = T.x1_int; 
	double y1 = T.y1_int;
	double x2 = T.x2_int; 
...
void InhomogeneousSingalCavity::ustar_int1(Triangle &T, Vector4d &u4, Vector4d &u5, Vector4d &u4_, Vector4d &u5_)
{
...
void InhomogeneousSingalCavity::ustar2_int1(Triangle &T, Vector4d &u5, Vector4d &u5_)
{
	double x1 = T.x1_intex;
	double y1 = T.y1_intex;
	double x2 = T.x2_intex;
...
void InhomogeneousSingalCavity::reloadtri(Triangle &T2, Triangle &T)
{
	T2.sgn_int = T.sgn_int;
	T2.x_int = T.x_int;
	T2.y_int = T.y_int;
...
void InhomogeneousSingalCavity::reloadtriex(Triangle &T2, Triangle &T)
{
...
Vector4cd InhomogeneousSingalCavity::solveweak4(Triangle &T, double v2, double v3)
{
	//phi = para.phi_int;    !!!!
...
//assign中需要使用的子函数
void InhomogeneousSingalCavity::assign_int(Triangle &T)
{
	if (abs(T.sgn_int) < 10)
	{
		int r = abs(T.sgn_int);
...
void InhomogeneousSingalCavity::assign_intex(Triangle &T)
{
